...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        super(MainWindow, self).__init__(parent)	#继承QMainwindow并重构
        ...
        # 数据导入 tab
        self.pushButton_openfile.clicked.connect(self.open_file)#点击连接def open_file
        ...
        # 绘图设置 tab
        self.radioButton_2D.toggled.connect(self.set_page_show)#轮流click连接,两个或多个函数之间的轮流切换
        ...
        self.comboBox_fitmethod.activated.connect(self.set_page_show)#激活连接
        ...
        ##tab4
            #划分
        self.comboBox_Mission.activated.connect(self.Mission)
            #Preprocessing
        self.comboBox_Impute_Method.activated.connect(self.Confirm)
        self.comboBox_encode.activated.connect(self.Confirm)
        self.comboBox_oversample.activated.connect(self.Confirm)
        self.comboBox_scale.activated.connect(self.Confirm)
            #Feature
        self.radioButton_Spearman.toggled.connect(self.Confirm)
        self.radioButton_Statistical.toggled.connect(self.Confirm)
        self.radioButton_rf.toggled.connect(self.Confirm)
        self.radioButton_rfe.toggled.connect(self.Confirm)
        self.radioButton_lasso.toggled.connect(self.Confirm)
        self.radioButton_mrmr.toggled.connect(self.Confirm)
        self.

        self.radioButton_C_dt.toggled.connect(self.Confirm)
        self.radioButton_C_knn.toggled.connect(self.Confirm)
        self.radioButton_C_SVM.toggled.connect(self.Confirm)
        self.radioButton_C_gbdt.toggled.connect(self.Confirm)
        self.radioButton_C_adaboot.toggled.connect(self.Confirm)
        self.radioButton_C_mlp.toggled.connect(self.Confirm)
        self.radioButton_C_rf.toggled.connect(self.Confirm)
        self.radioButton_C_etratree.toggled.connect(self.Confirm)
        self.radioButton_C_bagging.toggled.connect(self.Confirm)

        self.radioButton_R_dt.toggled.connect(self.Confirm)
        self.radioButton_R_knn.toggled.connect(self.Confirm)
        self.radioButton_R_SVM.toggled.connect(self.Confirm)
        self.radioButton_R_gbdt.toggled.connect(self.Confirm)
        self.radioButton_R_adaboot.toggled.connect(self.Confirm)
        self.radioButton_R_mlp.toggled.connect(self.Confirm)
        self.radioButton_R_rf.toggled.connect(self.Confirm)
        self.radioButton_R_etratree.toggled.connect(self.Confirm)
        self.radioButton_R_bagging.toggled.connect(self.Confirm)
        self.radioButton_R_lr.toggled.connect(self.Confirm)
        self.radioButton_R_naivebayes.toggled.connect(self.Confirm)

        self.pushButton_confirm.clicked.connect(self.Confirm)
        self.pushButton_browser.clicked.connect(self.browser)

    def Mission(self):
        mission=self.comboBox_Mission.currentIndex()
        if mission==0:
            self.groupBox_9.setEnabled(True)
            self.groupBox_10.setEnabled(False)
            Classification_list = [self.radioButton_C_dt, self.radioButton_C_knn, self.radioButton_C_SVM,
                                   self.radioButton_C_gbdt, self.radioButton_C_adaboot, self.radioButton_C_mlp,
                                   self.radioButton_C_rf, self.radioButton_C_etratree, self.radioButton_C_bagging]
            Classification_name = ['dt', 'knn', 'SVM',
                                   'gbdt', 'adaboot', 'mlp',
                                   'rf', 'etratree', 'bagging']
            for j in range(9):
                if Classification_list[j].ischecked():
                    classification = str(Classification_name[j])
            ...
            Regression_list = [self.radioButton_R_dt, self.radioButton_R_knn, self.radioButton_R_SVM,
                               self.radioButton_R_gbdt, self.radioButton_R_adaboot, self.radioButton_R_mlp,
                               self.radioButton_R_rf, self.radioButton_R_etratree, self.radioButton_R_bagging,
                               self.radioButton_R_lr, self.radioButton_R_naivebayes]
            Regression_name = ['dt', 'knn', 'SVM',
                               'gbdt', 'adaboot', 'mlp',
                               'rf', 'etratree', 'bagging',
                               'lr', 'naivebayes']
            for k in range(11):
                if Regression_list[k].ischecked():
                    regression = str(Regression_name[k])
    def Impute_Method(self):
        Impute=self.comboBox_impute.currentIndex()
        if Impute==0:
            impute_method='mean'
        elif Impute==1:
            impute_method='medianmost_frequent'
        elif Impute==2:
            impute_method = 'constant'
        else:
            impute_method = 'None'
        return str(impute_method)

    def Encode_Method(self):
        Encode=self.comboBox_encode.currentIndex()
        if Encode==0:
            ...
            encode_method = 'None'
        return str(encode_method)

    def Oversample_Method(self):
        Oversample=self.comboBox_oversample.currentIndex()
        if Oversample==0:
            oversample_method='SVM'
        elif Oversample==1:
            oversample_method='Borderline'
        elif Oversample==2:
        ...
        return str(oversample_method)

    def Scale_Method(self):
        Scale=self.comboBox_scale.currentIndex()
        if Scale==0:
        ...
        return str(scale_method)

    def Confirm(self):
        Feature_list=[self.radioButton_Spearman,self.radioButton_Statistical,
                     self.radioButton_rf,self.radioButton_rfe,self.radioButton_lasso]
        Feature_name=['Spearman','Statistical',
                      'rf','rfe','lasso']
        for i in range(5):
            if Feature_list[i].ischecked():
                feature=str(Feature_name[i])



        n_seed=self.lineEdit_n_seed.text()
        candidate_seed=self.lineEdit_candidate_seed.text()
        n_jobs=self.lineEdit_n_jobs.text()
        train_size=self.lineEdit_train_size.text()
        label_name=self.lineEdit_label_name.text()
        fill_value=self.lineEdit_fill_value.text()
        fold=self.lineEdit.text()
        max_iter=self.lineEdit.text()
        spearman_threshold=self.lineEidt.text()
